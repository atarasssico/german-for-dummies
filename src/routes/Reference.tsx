          ...
          <TabsList className="h-auto w-max gap-1 rounded-none bg-transparent p-0">
            {[
              ...
              <TabsTrigger
                key={value}
                value={value}
                className="shrink-0 rounded-none border-0 border-b-[1.5px] border-transparent px-3 py-2 text-[13px] tracking-wide text-muted-foreground shadow-none data-[state=active]:border-rule-strong data-[state=active]:bg-transparent data-[state=active]:font-medium data-[state=active]:text-foreground data-[state=active]:shadow-none"
              >
                {label}
              </TabsTrigger>
            ))}
      ...
      <table className="w-full min-w-[420px] border-collapse text-left">
        ...
        <thead>
          <tr className="border-b-[1.5px] border-rule-strong">
            <th scope="col" className="w-[7.5rem] pb-2 pr-3 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">
              Kasus
            </th>
            {SLOTS.map((slot) => (
              <th
                key={slot}
                scope="col"
                className="pb-2 pr-3 text-[11px] font-medium uppercase tracking-[0.12em]"
          ...
          {rows.map((row) => (
            <tr key={row.head} className="border-b border-rule">
              <th scope="row" className="py-2.5 pr-3 align-baseline">
                <span className="block text-[13px] font-medium">{row.head}</span>
                {row.sub && <span className="block text-[11px] text-muted-foreground">{row.sub}</span>}
              </th>
              {row.cells.map((cell, i) => (
                <td key={i} className="de py-2.5 pr-3 align-baseline text-[17px]" style={{ color: SLOT_HEAD[SLOTS[i] as Slot].colour }}>
                  {cell ?? '·'}
                </td>
              ))}
            </tr>
...
function DeterminerTables() {
  ...
  return (
    <section className="flex flex-col gap-4">
      ...
      <p className="text-[12.5px] text-muted-foreground">{set.note}</p>
      <GridTable
        caption={set.title}
    ...
    <section className="flex flex-col gap-4">
      ...
      <label className="flex items-center gap-3 border-b border-rule pb-2">
        ...
        <input
          ...
          spellCheck={false}
          className="de w-full border-b border-transparent bg-transparent text-[18px] outline-none focus-visible:border-foreground placeholder:text-muted-foreground/50"
        />
      </label>
        ...
        <ul className="flex flex-col border-b border-rule">
          {matches.map((n) => (
            <li key={n.id} className="border-b border-rule last:border-b-0">
              ...
              >
                <span className="de flex-1 text-[16px]" style={{ color: GENDER_VAR[n.gender] }}>
                  {({ m: 'der', f: 'die', n: 'das' } as Record<Gender, string>)[n.gender]} {n.word}
                </span>
                <span className="text-[12.5px] text-muted-foreground">{n.en}</span>
              </button>
            </li>
          ))}
        </ul>
      ...
      <Rail gender={target.gender} className="py-1">
        ...
        <p className="flex flex-wrap items-center gap-x-3 pt-2 text-[13px] text-muted-foreground">
          <span>{target.en}</span>
          <GenderChip gender={target.gender} />
          <span className="de text-[14px]">
        ...
        <table className="w-full min-w-[340px] border-collapse text-left">
          <thead>
            <tr className="border-b-[1.5px] border-rule-strong">
              ...
              <th scope="col" className="pb-2 pr-3 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">
                Singular
              </th>
              <th scope="col" className="pb-2 text-[11px] font-medium uppercase tracking-[0.12em] text-muted-foreground">
                Plural
              </th>
            </tr>
          </thead>
          <tbody>
            {KASUS.map((kasus) => (
              <tr key={kasus} className="border-b border-rule">
                <th scope="row" className="py-2.5 pr-3 align-baseline">
                  <span className="block text-[13px] font-medium">{KASUS_LABEL[kasus]}</span>
                  <span className="block text-[11px] text-muted-foreground">{KASUS_QUESTION[kasus]}</span>
                </th>
                <td className="de py-2.5 pr-3 align-baseline text-[17px]">{cell(kasus, 'sg')}</td>
                <td className="de py-2.5 align-baseline text-[17px]">{cell(kasus, 'pl') ?? '·'}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <section className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        ...
        <button
          ...
          className={cn(
            'h-9 w-fit border px-3 text-[12.5px] transition-colors',
      ...
      <ul className="long-list flex flex-col">
        {matches.map((verb) => (
          <li key={verb.id} className="border-b border-rule">
            ...
            >
              <span className="de min-w-0 flex-[1.1] text-[18px]">{verb.infinitive}</span>
              <span className="hidden flex-1 text-[12.5px] text-muted-foreground sm:block">{verb.en}</span>
  ...
  return (
    <div className="flex flex-col gap-10">
            ...
            {items.map((prep) => (
              <li key={prep.id} className="flex flex-col gap-2 border-b border-rule py-4">
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  <span className="de text-[21px] leading-none">{prep.word}</span>
                  <span className="text-[13px] text-muted-foreground">{prep.en}</span>
                </div>
                ...
                <ul className="flex flex-col gap-1.5">
                  {prep.examples.map((example) => (
                    <li key={example.de} className="flex flex-col gap-0.5">
                      <span className="de text-[15.5px]">
                        {example.de}
                        <span className="pl-2 font-sans text-[10.5px] uppercase tracking-[0.1em] text-muted-foreground">
                          {KASUS_LABEL[example.kasus]}
                        </span>
                      </span>
                      <span className="text-[12.5px] italic text-muted-foreground">{example.en}</span>
                    </li>
                  ))}
                </ul>

                {prep.note && (
                  <p className="text-[12px] leading-snug text-muted-foreground">{prep.note}</p>
                )}
              </li>
            ))}
          </ul>
      ...
      <section className="flex flex-col gap-1">
        ...
        <p className="py-3 text-[12.5px] leading-snug text-muted-foreground">
          ...
          {POSITION_PAIRS.map((pair) => (
            <li key={pair.id} className="grid gap-3 border-b border-rule py-4 sm:grid-cols-2">
              <div className="flex flex-col gap-1">
                ...
                <span className="de text-[14.5px]">{pair.example.dat.de}</span>
                <span className="text-[12px] italic text-muted-foreground">{pair.example.dat.en}</span>
              </div>
              <div className="flex flex-col gap-1">
                ...
                <span className="de text-[14.5px]">{pair.example.akk.de}</span>
                <span className="text-[12px] italic text-muted-foreground">{pair.example.akk.en}</span>
              </div>
            </li>
          ))}
        </ul>
...
function ValencyRow({ entry }: { entry: Valency }) {
  return (
    <li className="flex flex-col gap-2 border-b border-rule py-4">
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <span className="de text-[19px] leading-none">{entry.verb}</span>
        <span className="text-[13px] text-muted-foreground">{entry.en}</span>
      </div>
      <ul className="flex flex-col gap-2">
        {entry.frames.map((frame, i) => (
          <li key={i} className="flex flex-col gap-0.5 border-l-[3px] border-rule pl-3">
            <span className="flex flex-wrap items-baseline gap-x-2">
              <span className="text-[12.5px] font-medium">{frameText(frame)}</span>
              {frame.sense && <span className="text-[12.5px] text-muted-foreground">{frame.sense}</span>}
            </span>
            <span className="de text-[15.5px]">{frame.example.de}</span>
            <span className="text-[12px] italic text-muted-foreground">{frame.example.en}</span>
          </li>
        ))}
      </ul>
      {entry.note && <p className="text-[12px] leading-snug text-muted-foreground">{entry.note}</p>}
    </li>
  )
}
    ...
    <div className="flex flex-col gap-10">
      {sections.map((section) => (
        <section key={section.label} className="flex flex-col gap-1">
          ...
          {section.blurb && (
            <p className="py-3 text-[12.5px] leading-snug text-muted-foreground">{section.blurb}</p>
          )}
          <ul className="long-list flex flex-col">
